...
def resolve_instrument_identity(ticker: str) -> dict[str, Any]:
    ...
    for source_key, target_key in (
        ...
        if value is not None and target_key not in identity:
            identity[target_key] = value
    return identity
...
def build_instrument_context(
    ...
    quote_type = str((identity or {}).get("quote_type", "")).upper()
    if quote_type == "ETF":
        category = (identity or {}).get("category", "Unknown")
        parts = [f"category: {category}"]
        total_assets = (identity or {}).get("total_assets")
        if total_assets:
            with suppress(TypeError, ValueError):
                parts.append(f"AUM ~{float(total_assets):,.0f}")
        expense_ratio = (identity or {}).get("expense_ratio")
        if expense_ratio:
            with suppress(TypeError, ValueError):
                parts.append(f"expense ratio {float(expense_ratio):.2f}%")
        context += (
            f"\n\nIMPORTANT: `{ticker}` is an ETF; {', '.join(parts)}. "
            "Treat it as a basket, not an operating company: focus on holdings "
            "concentration, sector/thematic exposure, expense ratio, distribution "
            "yield, NAV premium/discount, and benchmark correlation. Use ETF-specific "
            "tools instead of single-company tools. Earnings, P/E, and insider "
            "transactions do not apply."
        )
    return context
